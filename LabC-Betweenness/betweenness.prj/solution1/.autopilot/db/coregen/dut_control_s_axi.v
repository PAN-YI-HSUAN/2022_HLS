...
#(parameter
    ...
    output wire [31:0]                   numVert,
    output wire [31:0]                   numEdge,
    output wire [63:0]                   offset,
    output wire [63:0]                   column,
    output wire [63:0]                   btwn,
    output wire [63:0]                   tmp0,
...
localparam
    ...
    ADDR_IER            = 7'h08,
    ADDR_ISR            = 7'h0c,
    ADDR_NUMVERT_DATA_0 = 7'h10,
    ADDR_NUMVERT_CTRL   = 7'h14,
    ADDR_NUMEDGE_DATA_0 = 7'h18,
    ADDR_NUMEDGE_CTRL   = 7'h1c,
    ADDR_OFFSET_DATA_0  = 7'h20,
    ADDR_OFFSET_DATA_1  = 7'h24,
    ADDR_OFFSET_CTRL    = 7'h28,
    ADDR_COLUMN_DATA_0  = 7'h2c,
    ADDR_COLUMN_DATA_1  = 7'h30,
    ADDR_COLUMN_CTRL    = 7'h34,
    ADDR_BTWN_DATA_0    = 7'h38,
    ADDR_BTWN_DATA_1    = 7'h3c,
    ADDR_BTWN_CTRL      = 7'h40,
    ADDR_TMP0_DATA_0    = 7'h44,
    ADDR_TMP0_DATA_1    = 7'h48,
    ADDR_TMP0_CTRL      = 7'h4c,
    ADDR_TMP1_DATA_0    = 7'h50,
    ADDR_TMP1_DATA_1    = 7'h54,
    ADDR_TMP1_CTRL      = 7'h58,
    ADDR_TMP2_DATA_0    = 7'h5c,
    ADDR_TMP2_DATA_1    = 7'h60,
    ADDR_TMP2_CTRL      = 7'h64,
    ADDR_TMP3_DATA_0    = 7'h68,
    ADDR_TMP3_DATA_1    = 7'h6c,
    ADDR_TMP3_CTRL      = 7'h70,
    WRIDLE              = 2'd0,
    WRDATA              = 2'd1,
    WRRESP              = 2'd2,
    WRRESET             = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_numVert = 'b0;
    reg  [31:0]                   int_numEdge = 'b0;
    reg  [63:0]                   int_offset = 'b0;
    reg  [63:0]                   int_column = 'b0;
    reg  [63:0]                   int_btwn = 'b0;
    reg  [63:0]                   int_tmp0 = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUMVERT_DATA_0: begin
                    rdata <= int_numVert[31:0];
                end
                ADDR_NUMEDGE_DATA_0: begin
                    rdata <= int_numEdge[31:0];
                end
                ...
                ADDR_COLUMN_DATA_1: begin
                    rdata <= int_column[63:32];
                end
                ADDR_BTWN_DATA_0: begin
                    rdata <= int_btwn[31:0];
                end
                ADDR_BTWN_DATA_1: begin
                    rdata <= int_btwn[63:32];
                end
                ADDR_TMP0_DATA_0: begin
                    rdata <= int_tmp0[31:0];
                end
                ADDR_TMP0_DATA_1: begin
                    rdata <= int_tmp0[63:32];
                end
...
assign numVert       = int_numVert;
assign numEdge       = int_numEdge;
assign offset        = int_offset;
assign column        = int_column;
assign btwn          = int_btwn;
assign tmp0          = int_tmp0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_NUMVERT_DATA_0)
            int_numVert[31:0] <= (WDATA[31:0] & wmask) | (int_numVert[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_NUMEDGE_DATA_0)
            int_numEdge[31:0] <= (WDATA[31:0] & wmask) | (int_numEdge[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COLUMN_DATA_1)
            int_column[63:32] <= (WDATA[31:0] & wmask) | (int_column[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BTWN_DATA_0)
            int_btwn[31:0] <= (WDATA[31:0] & wmask) | (int_btwn[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BTWN_DATA_1)
            int_btwn[63:32] <= (WDATA[31:0] & wmask) | (int_btwn[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TMP0_DATA_0)
            int_tmp0[31:0] <= (WDATA[31:0] & wmask) | (int_tmp0[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TMP0_DATA_1)
            int_tmp0[63:32] <= (WDATA[31:0] & wmask) | (int_tmp0[63:32] & ~wmask);
    end
end
